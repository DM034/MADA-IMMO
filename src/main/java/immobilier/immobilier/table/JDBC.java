 ...
 */
package immobilier.immobilier.table;
...
import immobilier.immobilier.connection.PostgresqlConnection;
...
public class JDBC {
    ...
    private Method getGetter(String field) {
        return getters.stream()
                .filter(prdct -> prdct.getAnnotation(Getter.class).columnName().toLowerCase().equals(field.toLowerCase()))
                .findFirst().get();
    }
    ...
    private Method getSetter(String field) {
        return setters.stream()
                .filter(prdct -> prdct.getAnnotation(Setter.class).columnName().toLowerCase().equals(field.toLowerCase()))
                .findFirst().get();
    }
    ...
    public boolean update(String field, String value) throws Exception {
        try {
            ...
            if (s > 0) {
                connection.commit();
                System.out.println("Data "+field+" updated");
                return true;
            } else {
                connection.rollback();
                System.out.println("Data "+field+" not updated");
                return false;
            }
        } catch (IllegalAccessException | NoSuchMethodException | InvocationTargetException | SQLException e) {
            connection.rollback();
